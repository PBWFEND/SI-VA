...
// 3. Menampilkan semua tugas menggunakan .forEach() dengan destructuring
console.log("\n--- Daftar Semua Tugas ---");
...
// 4. Menyaring tugas yang belum selesai menggunakan .filter() (implicit return)
console.log("\n--- Tugas yang Belum Selesai ---");
const tugasBelumSelesai = tasks.filter(task => !task.selesai);

// Menampilkan hasil filter (pakai destructuring juga)
tugasBelumSelesai.forEach(({ nama }) => console.log(`- ${nama}`));

// 5. Membuat daftar nama tugas saja menggunakan .map() (implicit return)
