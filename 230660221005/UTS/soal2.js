// soal2.js
const dataProduk = [
  { id: 1, nama: "Laptop Asus", harga: 12000000, kategori: "Elektronik" },
  { id: 2, nama: "Buku OOP", harga: 100000, kategori: "Buku" },
  { id: 3, nama: "Mouse Logitech", harga: 350000, kategori: "Elektronik" },
  { id: 4, nama: "Novel 'Bumi'", harga: 85000, kategori: "Buku" },
];

// Filter hanya produk Elektronik
const produkElektronik = dataProduk.filter((item) => item.kategori === "Elektronik");

// Buat daftar nama produk dengan format (ID: [id]) [Nama Produk]
const namaProduk = dataProduk.map((item) => `(ID: ${item.id}) ${item.nama}`);

console.log(produkElektronik);
console.log(namaProduk);
